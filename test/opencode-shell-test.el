...
(ert-deftest opencode-shell-entry-opens-explicit-server-without-directory-filter ()
  (let (opened)
  ...
  (let ((opencode-shell-profiles
    ...
    (unwind-protect
        (progn
          ...
          (should (commandp 'local-sessions))
          (should (commandp 'local-start))
          (should (commandp 'remote-sessions))
          (should (commandp 'remote-start))
          (cl-letf (((symbol-function 'opencode-shell--open-profile)
                     (lambda (profile &optional directory server-wide)
          ...
          (should-not (fboundp 'remote-start)))
...
(ert-deftest opencode-shell-register-profile-commands-does-not-overwrite-existing-functions ()
  (let ((opencode-shell-profiles
        ...
        (opencode-shell--generated-profile-commands nil))
    (cl-letf (((symbol-function 'reserved-sessions) #'ignore))
      (should-error (opencode-shell--register-profile-commands) :type 'user-error)
      (should (eq (symbol-function 'reserved-sessions) #'ignore)))))
...
(ert-deftest opencode-shell--sessions-supports-internal-directory-filter ()
  (let (seen)
    (cl-letf (((symbol-function 'pop-to-buffer) #'ignore)
