...
(ert-deftest opencode-shell-identical-active-poll-does-not-schedule-render ()
  (with-temp-buffer
    ...
    (let ((messages
           (list (opencode-shell-test--message "u1" "user" "question")
                 (opencode-shell-test--message "a1" "assistant" "partial" "u1" t)))
          (renders 0))
      (opencode-shell--render-messages messages 1)
      (let ((before (buffer-string))
            (tick (buffer-chars-modified-tick)))
        (cl-letf (((symbol-function 'opencode-shell--schedule-render)
                   (lambda (&rest _) (cl-incf renders))))
          (opencode-shell--render-messages messages 2 t)
          (should (= renders 0))
          (should (= opencode-shell--message-applied-sequence 2))
          (should (equal before (buffer-string)))
          (should (= tick (buffer-chars-modified-tick)))
          (opencode-shell--render-messages
           (list (opencode-shell-test--message "u1" "user" "question")
...
(ert-deftest opencode-shell-large-snapshot-updates-only-the-changed-response ()
  (with-temp-buffer
    ...
    (let* ((messages (opencode-shell-test--large-message-snapshot 40))
           (changed (copy-tree messages))
           (original (symbol-function 'opencode-shell--update-turn-response))
           (updates 0))
      (cl-letf (((symbol-function 'get-buffer-window) (lambda (&rest _) t))
        ...
        (cl-letf (((symbol-function 'opencode-shell--update-turn-response)
                   (lambda (turn)
                     (cl-incf updates)
                     (funcall original turn))))
          (opencode-shell--render-messages changed 2 t)
          (opencode-shell-async-drain (current-buffer)))
        (should (= updates 1))))))

(ert-deftest opencode-shell-spinner-overlay-invalidates-redisplay-without-buffer-edits ()
  (with-temp-buffer
    (opencode-shell-mode)
    (opencode-shell--render-messages
     (opencode-shell-test--large-message-snapshot 0) 1)
...
(ert-deftest opencode-shell-identical-human-snapshots-do-not-schedule-render ()
  (with-temp-buffer
    (opencode-shell-mode)
    (setq opencode-shell--session-id "s")
    (let ((permission '(((id . "p") (sessionID . "s") (permission . "bash"))))
          (question '(((id . "q") (sessionID . "s")
                       (questions . (((question . "Continue?")))))))
          (renders 0))
      (cl-letf (((symbol-function 'opencode-shell--schedule-render)
                 (lambda (&rest _) (cl-incf renders)))
                ((symbol-function 'opencode-shell--start-polling) #'ignore))
        (opencode-shell--receive-permissions permission t)
        (should (= renders 1))
        (opencode-shell--receive-permissions permission t)
        (should (= renders 1))
        (opencode-shell--receive-questions question t)
        (should (= renders 2))
        (opencode-shell--receive-questions question t)
        (should (= renders 2))
...
(ert-deftest opencode-shell-runtime-shares-one-stream-cadence-and-cleans-up ()
  (let ((opencode-shell-async--runtimes (make-hash-table :test #'equal))
        ...
        (second (generate-new-buffer " *runtime-second*"))
         timers cancelled (starts 0) (stops 0))
    (unwind-protect
        (cl-letf (((symbol-function 'opencode-shell-sse-start)
                   (lambda (&rest _)
                     (cl-incf starts)
                     (opencode-shell-sse--make-connection :state 'connecting)))
                  ((symbol-function 'opencode-shell-sse-stop)
                   (lambda (connection) (when connection (cl-incf stops))))
                  ((symbol-function 'run-at-time)
                   (lambda (&rest args)
                     (let ((timer (cons 'timer args)))
                       (push timer timers)
...
(ert-deftest opencode-shell-sse-rejects-unsafe-and-conflicting-config ()
  (let ((opencode-shell-async--runtimes (make-hash-table :test #'equal))
    ...
    (cl-letf (((symbol-function 'opencode-shell-sse-start)
               (lambda (_url _headers _event error &rest _)
                 (setq made t)
                 (funcall error '(:type config :reason unsafe))
                 (opencode-shell-sse--make-connection :state 'closed)))
              ((symbol-function 'run-at-time) (lambda (&rest _) 'timer))
              ((symbol-function 'timerp) (lambda (value) (eq value 'timer))))
      (opencode-shell-async-subscribe-runtime
...
(ert-deftest opencode-shell-sse-runtime-opens-circuit-after-bounded-failures ()
  (let* ((key 'circuit)
         (runtime (list :subscribers (make-hash-table :test #'eq)
                        :attempt 'attempt :failures 4 :backoff 1
                        :reconnect-timer nil :sse-disabled nil)))
    (puthash (current-buffer) #'ignore (plist-get runtime :subscribers))
    (puthash key runtime opencode-shell-async--runtimes)
    (unwind-protect
        (progn
          (opencode-shell-async--transport-error
           key 'attempt '(:type transport :reason closed))
          (should (plist-get runtime :sse-disabled))
...
(ert-deftest opencode-shell-async-rejects-nonpositive-runtime-intervals ()
  (dolist (interval '(0 -1))
    (should-error
     (opencode-shell-async-subscribe-runtime
      'invalid (current-buffer) "http://localhost/event" nil nil
      interval #'ignore))
    (should-error
     (opencode-shell-async-subscribe-animation
      (current-buffer) interval #'ignore))))
